...
exports.buildGovernanceReviewArtifact = buildGovernanceReviewArtifact;
exports.writeGovernanceReview = writeGovernanceReview;
exports.readGovernanceReview = readGovernanceReview;
exports.formatReviewForInject = formatReviewForInject;
exports.formatGovernanceReviewYaml = formatGovernanceReviewYaml;
exports.buildGovernanceRulesLines = buildGovernanceRulesLines;
exports.estimateTokens = estimateTokens;
exports.formatGovernanceExportSection = formatGovernanceExportSection;
exports.computeGovernanceScore = computeGovernanceScore;
const fs = __importStar(require("node:fs/promises"));
const path = __importStar(require("node:path"));
const governanceInject_js_1 = require("../prompt-engine/governanceInject.js");
const GOVERNANCE_DIR = '.contora/governance';
const REVIEW_FILE = 'review.json';
const LEGACY_REVIEW_FILE = 'governance-review.json';
function primaryReviewPath(workspaceRoot) {
    return path.join(workspaceRoot, GOVERNANCE_DIR, REVIEW_FILE);
}
function legacyReviewPath(workspaceRoot) {
    return path.join(workspaceRoot, GOVERNANCE_DIR, LEGACY_REVIEW_FILE);
}
function buildGovernanceReviewArtifact(result, filePath, opts) {
    ...
    const truthImpact = (guard.detections ?? []).some((d) => d.type === 'truth_registry' || d.type === 'hardcode_snippet' || d.type === 'hardcoded_value');
    const artifact = {
        version: 2,
...
function mapLegacyRisk(action, riskLevel) {
    if (action === 'block') {
        return 'critical';
    }
    if (riskLevel === 'high') {
...
function displayScoreForRisk(risk) {
    switch (risk) {
        case 'low':
            return 88;
        case 'medium':
...
function parseProtectedLevel(chain) {
...
function normalizeReviewArtifact(parsed) {
...
async function writeGovernanceReview(workspaceRoot, artifact) {
...
async function readGovernanceReview(workspaceRoot) {
    for (const target of [primaryReviewPath(workspaceRoot), legacyReviewPath(workspaceRoot)]) {
        try {
            ...
            const migrated = migrateV1Review(parsed);
            if (migrated) {
                return migrated;
            }
        }
...
function migrateV1Review(old) {
    if (typeof old.file !== 'string') {
        return null;
    }
    const riskRaw = String(old.risk ?? 'low');
    const risk = riskRaw === 'critical' ? 'critical' : riskRaw === 'high' ? 'high' : riskRaw === 'medium' ? 'medium' : 'low';
    const display_score = typeof old.display_score === 'number'
        ? old.display_score
    ...
    return {
        ...
        file: old.file,
        status: old.status ?? 'pass',
        risk,
        change_type: old.change_type ?? 'unknown',
        severity: old.severity ?? 'low',
        impact: old.impact ?? 'none',
        ...
        display_score,
        review_source: old.review_source ?? 'static_file',
        review_timestamp: typeof old.review_timestamp === 'string'
            ? old.review_timestamp
            : new Date(typeof old.generatedAt === 'number' ? old.generatedAt : Date.now()).toISOString(),
        review_scope: old.review_scope ?? 'current_file',
        staged_files: Array.isArray(old.staged_files) ? old.staged_files : undefined,
    };
}
function formatReviewForInject(review) {
    const lines = [
        'Review Result:',
        '',
    ...
    if (review.staged_files?.length) {
        lines.splice(6, 0, `Staged files (${review.staged_files.length}): ${review.staged_files.slice(0, 6).join(', ')}`);
    }
    return lines.join('\n');
}
function formatGovernanceReviewYaml(review) {
    return [
        'GOVERNANCE REVIEW:',
        '',
...
async function buildGovernanceRulesLines(workspaceRoot, activeFile) {
    const relevant = await (0, governanceInject_js_1.getRelevantGovernanceForFile)(workspaceRoot, activeFile);
    const lines = [];
...
async function formatGovernanceExportSection(workspaceRoot, review) {
    const relevant = await (0, governanceInject_js_1.getRelevantGovernanceForFile)(workspaceRoot, review?.file);
...
function computeGovernanceScore(input) {
