...
exports.isStoryIdentityQuery = isStoryIdentityQuery;
exports.isDirectionQuery = isDirectionQuery;
exports.isDriftQuery = isDriftQuery;
exports.extractWhatIsEntityTopic = extractWhatIsEntityTopic;
/** Story / narrative identity — route to Story engine, not PIK. */
function isStoryIdentityQuery(question) {
    const q = question.toLowerCase().trim();
    return (/^what(?:'s| is)\s+(?:this|the)\s+project\??$/.test(q) ||
...
function isDirectionQuery(question) {
    ...
    return (/core direction|project direction|where (are we|is (this )?project) going|primary goal|project goal|main goal|project identity|why does this project exist|what are we building|north star|mission|vision|偏离|核心方向|项目目标|我们在做什么/.test(q) || /^what is the (core direction|main goal|primary goal|mission|vision)\b/.test(q));
}
...
function extractWhatIsEntityTopic(question) {
