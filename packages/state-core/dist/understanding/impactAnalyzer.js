...
function moduleOfSymbol(ref) {
    const idx = ref.indexOf('::');
    return idx >= 0 ? ref.slice(0, idx) : ref;
}
function fnNameOfSymbol(ref) {
    const parts = ref.split('::');
    return parts.length >= 2 ? parts[parts.length - 1] : ref;
}
function inferRisk(affectedCount, names) {
...
function analyzeImpact(graph, change) {
    const changedFnNames = new Set(change.key_changes.filter((k) => k.kind === 'function').map((k) => fnNameOfSymbol(k.symbol)));
    const changedFnIds = new Set(graph.nodes
