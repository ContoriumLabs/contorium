...
exports.formatProjectSnapshotMarkdown = formatProjectSnapshotMarkdown;
exports.projectSnapshotBulletLines = projectSnapshotBulletLines;
function section(title, items, empty = '(none yet)') {
    const lines = [title + ':'];
    if (!items.length) {
        lines.push(empty);
        return lines;
...
function formatProjectSnapshotMarkdown(state) {
    const lines = ['PROJECT SNAPSHOT', ''];
    lines.push('Goal:');
    lines.push(state.project_goal.trim() ||
...
function projectSnapshotBulletLines(state) {
    const out = [];
