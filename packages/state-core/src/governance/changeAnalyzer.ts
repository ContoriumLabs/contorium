...
function diffChangedLines(diff: string): string[] {
  return diff
    .split('\n')
...
function isStyleOnlyDiff(diff: string): boolean {
...
function pathMatches(path: string | undefined, patterns: RegExp[]): boolean {
  if (!path) {
    return false;
  }
...
function classifyProbe(probe: string, path?: string): ChangeType[] {
  const hints: ChangeType[] = [];
...
function pickPrimaryType(hints: ChangeType[], diff: string, path?: string): ChangeType {
  const priority: ChangeType[] = [
    'security',
    'database',
    'architecture',
    'api',
...
function severityForType(changeType: ChangeType, linesAdded: number, linesRemoved: number): ChangeSeverity {
...
export function computeChangeConfidence(change: Pick<ChangeAnalysis, 'change_type' | 'mixed' | 'lines_added' | 'lines_removed' | 'type_hints'>): number {
...
export function analyzeChange(input: ChangeAnalysisInput): ChangeAnalysis {
  ...
  const mixed =
    uniqueHints.filter((t) => !['comment', 'style'].includes(t)).length > 1 ||
