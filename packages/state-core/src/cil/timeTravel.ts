...
import { findSnapshotByDate } from './snapshotEngine.js';
import type { AdrRecord, TimeTravelPerspective, TimeTravelResult } from './types.js';

function onOrBefore(isoDate: string, targetDate: string): boolean {
 ...
 */
export async function queryTimeTravel(
  ...
  const snapshot = await findSnapshotByDate(workspaceRoot, date);
  ...
  let events =
    perspective === 'historical' && snapshot?.events?.length
      ? snapshot.events
      : allEvents.filter((e) => onOrBefore(e.timestamp, date)).slice(0, 24);

  let decisions: AdrRecord[] =
    perspective === 'historical' && snapshot?.decisions?.length
      ? snapshot.decisions
      : allAdrsCurrent.filter((a) => onOrBefore(a.date, date));

  const retrospective_notes: string[] = [];
  ...
  if (perspective === 'retrospective') {
    ...
    for (const adr of decisions) {
      if (adr.superseded_by) {
        ...
        retrospective_notes.push(
          `${adr.id} (${adr.title}) — now known superseded by ${adr.superseded_by}${repl ? `: ${repl.title}` : ''}`,
        );
      }
  ...
  const focus =
    (perspective === 'historical' ? snapshot?.state.current_task : undefined)?.trim() ||
  ...
  formatted.push(
    ...
    ...events.slice(0, 10).map((e) => `  · ${e.timestamp.slice(0, 10)}: ${e.title}`),
  );
