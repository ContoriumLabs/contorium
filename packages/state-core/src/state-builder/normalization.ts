...
function compressSimilarActions(actions: readonly string[], taskAnchor: string): string[] {
...
function stripTaskFromGoal(goal: string, taskAnchor: string): string {
...
function stripTaskFromStage(stage: string, taskAnchor: string): string {
...
export function normalizeProjectBuiltState(
  raw: ProjectBuiltState,
  taskAnchor: string,
): ProjectBuiltState {
  ...
  const problems = dedupeStrings(
    raw.open_problems.filter((p) => !echoesTask(p, taskAnchor)),
    6,
  );
  const milestones = dedupeStrings(raw.completed_milestones, 5);
  const decisions = dedupeStrings(
    raw.recent_decisions.filter((d) => !echoesTask(d, taskAnchor)),
    8,
  );
  const next_actions = compressSimilarActions(
    raw.next_actions.filter((a) => !echoesTask(a, taskAnchor)),
    taskAnchor,
  );
...
export function filterWeakInferenceLines(lines: readonly string[], taskAnchor: string): string[] {
