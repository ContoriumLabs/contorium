import {
  HOTSPOT_SCORE_ACTIVE,
  HOTSPOT_SCORE_COOLING,
...
function hotspotLifecycle(score: number): HotspotLifecycle {
  if (score >= HOTSPOT_SCORE_ACTIVE) {
    return 'active';
  }
  if (score >= HOTSPOT_SCORE_COOLING) {
    return 'cooling';
  }
  return 'stale';
}
...
export function buildHotspots(input: HotspotBuildInput): HotspotNode[] {
  ...
  const consider = (args: {
    ...
    const score =
      Math.round(
        (editFrequency * 0.4 + gitActivity * 0.2 + intentWeight * 0.3 + depNorm * 0.1) * 1000,
      ) / 1000;
  ...
  return hotspots
    ...
    .slice(0, input.max ?? SNAPSHOT_TOP_HOTSPOTS);
}
