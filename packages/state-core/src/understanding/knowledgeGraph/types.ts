...
export const KNOWLEDGE_ENGINE_VERSION = '3.1.0';
...
export interface LegacyProjectKnowledgeGraph {
  ...
  generatedAt: number;
  nodes: KnowledgeNode[];
  edges: Array<Omit<KnowledgeEdge, 'confidence'> & { confidence?: number }>;
  intentMappings: Array<Omit<IntentFunctionMapping, 'confidence'> & { confidence?: number }>;
  reasonTraces: ReasonTraceItem[];
  intentTrees: KnowledgeGraphTreeNode[];
}
