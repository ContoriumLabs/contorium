...
export function normalizeGraphRef(ref: string): string {
  const t = ref.trim();
  if (!t) {
    return t;
  }
...
export function formatNextActionPlain(a: HandoffNextAction): string {
...
function formatSnapshotFromBuilt(built: ProjectBuiltState): string[] {
  const lines: string[] = [];
  ...
  if (built.completed_milestones.length) {
    lines.push('');
    lines.push(
      ...listSection('milestones:', built.completed_milestones.slice(0, 8).map((m) => m.slice(0, 120))),
    );
  }
  if (built.open_problems.length) {
    lines.push('');
    lines.push(
      ...listSection('open_problems:', built.open_problems.slice(0, 6).map((p) => p.slice(0, 160))),
    );
  }
...
function formatSnapshotFromMarkdown(md: string): string[] {
  ...
  const pushItem = (key: keyof typeof buckets, item: string) => {
    const t = item.trim();
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (t.startsWith('- ')) {
      ...
      if (section && section in buckets) {
        pushItem(section as keyof typeof buckets, item);
      }
      continue;
    }
...
function deriveExecutionFocus(handoff: HandoffArtifact): string {
...
export function formatAiHandoffExecutionBlock(handoff: HandoffArtifact): string {
  const lines: string[] = ['# AI HANDOFF (V3.1)'];
  const goal = handoff.goal.trim();
...
function collectModifiedSymbols(handoff: HandoffArtifact): string[] {
  const out: string[] = [];
  const seen = new Set<string>();
  for (const k of handoff.key_changes) {
...
function formatCognitiveSnapshotSection(snap: KnowledgeSnapshot): string[] {
 ...
 */
export function formatCanonicalAiMarkdown(input: CanonicalAiExportInput): string {
  const lines: string[] = [];
  ...
  lines.push(
    ...listSection(
      'recent_git_activity:',
      input.recentGitActivity.length ? input.recentGitActivity : ['(none)'],
    ),
  );
  lines.push('');
  ...
  if (handoff?.impact_summary.affected_functions.length) {
    ...
    lines.push(
      ...listSection(
        'impacted_symbols:',
        handoff.impact_summary.affected_functions.slice(0, 12),
      ),
    );
    lines.push('');
  }
