...
export function appendLifecycleTrustOverlay(
  formatted: string[],
  ...
  const flagged = index.decisions.filter(
    (r) => r.needs_review || r.formatted_warnings.length || r.lifecycle_status !== 'ACTIVE',
  );
  ...
  for (const r of show) {
    ...
    lines.push(
      `${marker} ${r.title}`,
      `  Status ${r.lifecycle_status} | Trust ${r.confidence.overall}% | Freshness ${r.freshness_score}%`,
    );
  ...
  if (index.review_queue.length) {
    lines.push(
      `Ask "What needs review?" or run \`contorium review\` for ${index.review_queue.length} queued item(s).`,
    );
  }
...
export function formatSupersededDecisionPreamble(
  record: Pick<
    import('./types.js').DecisionLifecycleRecord,
    'title' | 'lifecycle_status' | 'created_at' | 'superseded_by' | 'evolution_chain'
  >,
): string | undefined {
  ...
  const chain =
    record.evolution_chain.length > 1
