import type { AdrRecord, DecisionContradiction } from '../cil/types.js';
import type { DecisionLifecycleMeta, KnowledgeConfidenceDimensions } from './types.js';
import { mapAdrToLifecycleStatus } from './evolution.js';
...
export function computeDecisionConfidence(
  ...
  const source = clamp100(
    (adr.reason.length > 20 ? 40 : 20) +
  ...
  const conflict = clamp100(
    conflictRefs.length
      ? Math.max(5, 100 - conflictRefs.length * LIFECYCLE_POLICY.confidenceConflictPenalty)
      : 100,
  );
  ...
  const verification = clamp100(
    meta.verification_type === 'manual'
      ? 100
  ...
  const lc = mapAdrToLifecycleStatus(adr.status);
  const consistency = clamp100(
    lc === 'ACTIVE' ? 100 : lc === 'SUPERSEDED' ? 35 : lc === 'DEPRECATED' ? 15 : 50,
  );
