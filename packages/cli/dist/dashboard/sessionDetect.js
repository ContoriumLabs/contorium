...
async function latestEventTimestamp(workspaceRoot) {
...
export async function detectIdeSession(workspaceRoot) {
    ...
    const lastUpdatedMs = state.source?.lastUpdated
        ...
        : state.lastUpdated;
    const recentState = Number.isFinite(lastUpdatedMs) && now - lastUpdatedMs < SESSION_ACTIVE_MS;
    const ideWriter = state.source?.lastWriter === 'ide';
    const eventMode = state.source?.mode === 'event-driven' || state.source?.mode === 'merged';
    if (recentEvent) {
