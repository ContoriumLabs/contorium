...
function isListSectionHeader(line) {
    return TWO_COL_SECTIONS.has(line.trim());
}
function isNonListLine(line) {
    const t = line.trim();
    if (!t) {
        return true;
    }
...
export function trimHistoryEvents(lines, maxEvents) {
    const header = [];
    const events = [];
    let i = 0;
...
export function layoutHistoryTwoColumn(lines, width) {
    const out = [];
...
function streamBlock(title, body, c, width) {
    const lines = [truncate(c.bold(title), width)];
    if (!body.length) {
...
export function renderHistoryStreams(historyLines, ctx) {
