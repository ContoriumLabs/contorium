import { readStateJson, readWorkspaceStatus, readProjectBuiltState, readIntentGraphVNext, readGovernanceDecision, readDecisionProvenanceGraph, readProjectEvolutionTimeline, readProjectGraph, readConfidenceIndex, readWhyLayer, readProjectIntelligenceHealth, deriveProjectIntelligenceHealth, readDecisionLog, readHandoffArtifact, readProjectKnowledgeGraph, setGitSubprocessAllowed, syncWorkspaceState, loadTransferExportInput, buildTransferContextSnapshot, buildFullIntelligenceMarkdown, formatTransferContextMarkdown, formatTransferContextJson, finalizeTransferContextText, getProjectHandoff, captureProjectFocus, captureProjectNote, captureProjectDecision, retrieveEvolution, retrieveProvenance, retrieveImpact, } from '@contora/state-core';
...
async function cmdInspectTarget(root, target) {
    switch (target) {
        case 'state': {
...
async function cmdTransferMode(root, mode) {
    ...
    if (mode === 'story' || mode === 'essence') {
        const { syncCognitiveInteractionLayer, runCognitiveKernel, setGitSubprocessAllowed, syncWorkspaceState } = await import('@contora/state-core');
        setGitSubprocessAllowed(true);
...
async function cmdCaptureKind(root, kind) {
    switch (kind) {
        case 'focus': {
...
export async function cmdPil(root, group) {
