...
function copyWindowsSync(text) {
    ...
    try {
        const ps = spawnSync('powershell', ['-NoLogo', '-NoProfile', '-NonInteractive', '-Command', '[Console]::In.ReadToEnd() | Set-Clipboard'], { input: text, encoding: 'utf8', windowsHide: true, timeout: 8000 });
        return ps.status === 0;
    }
    catch {
        return false;
...
function copyWindowsAsync(text) {
