import { DEFAULT_LLM_CONFIG, getAiStatus, readLlmConfig, testAiConnection, writeLlmConfig, } from '@contora/state-core';
...
export async function cmdAi(root, sub) {
    switch (sub) {
        case 'setup': {
            const provider = (flagValue('--provider') ?? DEFAULT_LLM_CONFIG.provider);
            const model = flagValue('--model') ?? DEFAULT_LLM_CONFIG.model;
            const enable = hasFlag('--enable');
            const routerMode = (flagValue('--router') ?? 'hybrid');
