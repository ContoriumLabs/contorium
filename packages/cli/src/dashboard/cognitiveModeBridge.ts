...
export async function readDashboardCognitiveHotkey(
  workspaceRoot: string,
): Promise<{ panel_key: string; panel_accelerator: string }> {
  ...
  try {
    const raw = JSON.parse(
      await fs.readFile(path.join(workspaceRoot, '.contora/mcp/cognitive.hotkey.json'), 'utf8'),
    ) as { panel_key?: string; panel_accelerator?: string };
    return {
...
function migrateMode(raw: { mode?: string; schemaVersion?: number }): ContoriumMcpMode {
...
export async function readDashboardCognitiveMode(workspaceRoot: string): Promise<ContoriumMcpMode> {
  try {
    const raw = JSON.parse(
      await fs.readFile(path.join(workspaceRoot, '.contora/mcp/cognitive.mode.json'), 'utf8'),
    ) as { mode?: string; schemaVersion?: number };
...
export async function applyCognitiveModeFromDashboard(
  workspaceRoot: string,
  ...
  if (applyPath) {
    try {
      ...
      const mod = (await import(pathToFileURL(applyPath).href)) as {
        applyCognitiveModeChange: (
          root: string,
          m: ContoriumMcpMode,
          source: string,
        ) => Promise<unknown>;
  ...
  await fs.writeFile(
    fp,
    `${JSON.stringify({ mode, updatedAt: new Date().toISOString(), source: 'user' }, null, 2)}\n`,
    'utf8',
  );
  return {
    ok: true,
...
export function isCtrlAltC(raw: string): boolean {
