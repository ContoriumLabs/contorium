...
function impactGraphLines(
  ...
  if (ug?.call_chain.length) {
    ...
    if (ug.affected.length > ug.call_chain.length) {
      lines.push(
        truncate(
          `${c.dim('影响:')} ${ug.affected.slice(ug.call_chain.length, ug.call_chain.length + 6).join(', ')}`,
          width - 2,
        ),
      );
    }
...
function agentTimelineLines(state: DashboardState, c: Record<string, ColorFn>, width: number): string[] {
  ...
  for (const kc of keyChanges(state).slice(0, 3)) {
    lines.push(
      truncate(`${writer}: function_update ${kc.symbol}`, width - 2),
    );
  }
...
export function renderPassiveLine(
  ...
  const badge =
    updateCount > 0 && !chp?.recent_changes.length
      ? c.dim(` (+${updateCount})`)
      : '';
...
export function renderExpanded(state: DashboardState, ctx: RenderContext): string {
  ...
  const header = [
    ...
    c.dim(
      `updated ${new Date(state.loadedAt).toLocaleTimeString()} · Space toggle · c Copy To AI · q quit · fullscreen`,
    ),
    '',
  ];
  ...
  const leftSections = wide
    ? [
        ...section(
          'Impact Graph',
          impactGraphLines(state, c, colW, graphRows),
          colW,
        ),
        ...section(
          'Agent Timeline',
          agentTimelineLines(state, c, colW).slice(0, timelineRows),
          colW,
        ),
      ]
    : [];
  ...
  const rightSections = [
    ...section(
      'Function Updates',
      functionUpdateLines(state, c, colW, ctx.filter).slice(0, fnRows),
      colW,
    ),
    ...(wide
      ? []
