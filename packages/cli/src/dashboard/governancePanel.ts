...
function decisionStyled(action: string, c: Record<string, ColorFn>): string {
...
export function renderGovernanceSummaryLines(
...
export function renderDecisionFeedLines(
...
export function renderScopeMapLines(
  gov: DashboardGovernanceSnapshot | undefined,
  useColor: boolean,
  width: number,
): string[] {
  ...
  const s = gov.scope;
  const row = (label: string, files: string[]) => {
    if (!files.length) {
...
export function renderDecisionTraceLines(
  gov: DashboardGovernanceSnapshot | undefined,
  useColor: boolean,
  width: number,
): string[] {
...
export function renderGovernanceRawLines(
  gov: DashboardGovernanceSnapshot | undefined,
  useColor: boolean,
  width: number,
): string[] {
