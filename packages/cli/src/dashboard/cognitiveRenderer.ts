/**
 * Cognitive State Renderer — PIL-aligned terminal dashboard.
 * Single cognitive flow: Core → Dimensions (STATE/INTENT/DECISION/WHY) → Streams → View Mode.
 */
...
function evolutionStreamLines(
  ...
  if (timeline.length) {
    return timeline.map((e, i) =>
      truncate(`→ v${i + 1} (${e.event_type}: ${e.entity_id})`, width),
    );
  }
