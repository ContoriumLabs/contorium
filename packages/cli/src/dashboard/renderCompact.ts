...
function colors(enabled: boolean): Record<string, ColorFn> {
  const wrap =
    (code: string): ColorFn =>
    (text) =>
...
function riskStyled(risk: string, level: 'low' | 'medium' | 'high', c: Record<string, ColorFn>): string {
  if (level === 'high') {
    return c.red(risk);
  }
  if (level === 'medium') {
...
export function renderCompactView(state: DashboardState, ctx: RenderContext): string[] {
  ...
  const section = (title: string) => animatedSectionHeader(title, tick, c);
