...
export function renderIntelligenceHealthLines(
  health: ProjectIntelligenceHealth | undefined,
  useColor: boolean,
  width: number,
): string[] {
...
export function renderKnowledgeCoverageLines(
  health: ProjectIntelligenceHealth | undefined,
  useColor: boolean,
  width: number,
): string[] {
  ...
  if (detail?.total_modules.length) {
    lines.push(
      truncate(
        `${detail.covered_modules.length}/${detail.total_modules.length} modules covered`,
        width,
      ),
    );
...
export function renderTimelineVizLines(
  timeline: ProjectEvolutionTimeline | undefined,
  useColor: boolean,
  width: number,
): string[] {
  const c = colors(useColor);
...
export function renderEvolutionVizLines(
  graph: EvolutionGraphArtifact | undefined,
  useColor: boolean,
  width: number,
): string[] {
  const c = colors(useColor);
...
export function renderProvenanceExplorerLines(
  provenance: ProvenanceChainArtifact | undefined,
  useColor: boolean,
  width: number,
): string[] {
  const c = colors(useColor);
...
export function renderImpactVizLines(
  impact: ImpactGraphArtifact | undefined,
  useColor: boolean,
  width: number,
): string[] {
  const c = colors(useColor);
