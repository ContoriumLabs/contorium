import { formatValidityStateLabel, type KnowledgeLifecycleIndex } from '@contora/state-core';
import type { DashboardState } from './types.js';
import { truncate } from './uiHelpers.js';

function reviewItemLabel(
  item: NonNullable<KnowledgeLifecycleIndex['review_queue']>[number],
): string {
...
function topValiditySignal(
  record: KnowledgeLifecycleIndex['decisions'][number],
): string | undefined {
...
export function renderKnowledgeGovernanceLines(
  ...
  for (const r of flaggedValidity) {
    ...
    lines.push(
      `⚠ ${r.title} — ${formatValidityStateLabel(r.validity_state)} · trust ${r.confidence.overall}%`,
    );
    if (why) {
      lines.push(`  ${why}`);
    }
...
export function knowledgeLifecycleSummary(
  lc: KnowledgeLifecycleIndex | null | undefined,
): string {
  if (!lc?.health) {
    return 'Lifecycle: —';
  }
...
export function renderLayeredHealthLines(state: DashboardState, width: number): string[] {
  const lines: string[] = [];
  const lc = state.knowledgeLifecycle;
  ...
  if (pil) {
    lines.push(
      `Project Intelligence: ${Math.round(pil.health_score * 100)}% ${pil.health_category ?? ''}`.trim(),
    );
  }
