...
async function lockAgeMs(workspaceRoot: string): Promise<number | undefined> {
...
async function clearExpiredSpawnLock(workspaceRoot: string): Promise<void> {
...
export async function tryAcquireDashboardSpawnLock(workspaceRoot: string): Promise<boolean> {
  const fp = lockPath(workspaceRoot);
...
export async function isDashboardSpawnPending(workspaceRoot: string): Promise<boolean> {
  await clearExpiredSpawnLock(workspaceRoot);
  const age = await lockAgeMs(workspaceRoot);
  if (age === undefined || age >= LOCK_TTL_MS) {
    return false;
...
export async function releaseDashboardSpawnLock(workspaceRoot: string): Promise<void> {
  try {
