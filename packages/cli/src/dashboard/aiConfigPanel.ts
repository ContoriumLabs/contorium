...
export function renderLlmConfigStreams(
  ...
  if (args.step === 'provider') {
    ...
    if (snap) {
      ...
      lines.push(
        truncate(
          `Active: ${activeLabel} · ${snap.model}${snap.enabled ? c.green(' · on') : c.dim(' · off')} · Key: ${snap.activeProviderHasKey ? c.green('configured') : c.dim('(not set)')}`,
          w,
        ),
      );
    }
  ...
  const visible =
    args.keyInputBuffer.length > 0 ? args.keyInputBuffer : c.dim('(typing…)');
