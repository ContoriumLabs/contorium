...
function positionalTextAfterCommand(): string {
...
export async function cmdCheckAction(root: string): Promise<void> {
  const control = createControlSurface(root, 'cli');
...
export async function cmdUpdateProjectIntent(root: string): Promise<void> {
...
export async function cmdControl(root: string, sub: string | undefined): Promise<void> {
  const control = createControlSurface(root, 'cli');
