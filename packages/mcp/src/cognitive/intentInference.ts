...
const KEYWORD_RULES: Array<{ re: RegExp; intent: string; weight: number; signal: string }> = [
...
function inferActionPattern(ctx: WorkspaceContext): IntentInference['action_pattern'] {
...
export function inferIntent(ctx: WorkspaceContext): IntentInference {
  const blob = [
    ctx.currentTask,
    ctx.focusHint,
    ctx.projectType,
...
export function generateKeywords(ctx: WorkspaceContext, intent: IntentInference): string[] {
  ...
  const domainBoost: Record<string, string[]> = {
