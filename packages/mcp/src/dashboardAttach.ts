...
function quoteBatPath(value: string): string {
  return `"${value.replace(/"/g, '""')}"`;
}

function spawnHiddenNode(args: string[], cwd: string): void {
...
export function spawnDashboardWindow(workspaceRoot: string): boolean {
  const cli = resolveContoriumCliBinary();
  if (!cli) {
    console.error(
      '[contorium-mcp] CLI not found — set CONTORIUM_CLI_PATH or use local node MCP (not npx). See commands/setup-mcp-codex.md',
    );
    return false;
  }
...
export async function attachDashboardFallback(workspaceRoot: string): Promise<void> {
