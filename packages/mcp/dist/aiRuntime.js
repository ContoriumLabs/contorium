 ...
 */
export function registerAiRuntimeTools(server, resolveRoot) {
    server.registerTool('get_ai_status', {
        description: '[CIL · AI Layer] LLM explanation-layer status — enabled flag, provider, model, module switches, intent router mode. Does not expose API keys.',
    ...
    server.registerTool('test_ai_connection', {
        description: '[CIL · AI Layer] Test LLM provider connectivity using workspace llm.json + api_key_env. Returns ok/latency/message.',
        inputSchema: workspaceRootSchema,
    }, async ({ workspaceRoot: override }) => {
        const root = override ?? (await resolveRoot());
