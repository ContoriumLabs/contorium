import { z } from 'zod';
import { buildCognitiveInsights, observationOnlyPayload, readCognitiveInsights, } from './cognitiveOverlay.js';
...
export function registerCognitiveTools(server, resolveRoot) {
    server.registerTool('get_cognitive_mode', {
    ...
    server.registerTool('set_cognitive_mode', {
        ...
        const root = override ? override : await resolveRoot();
        const result = await applyCognitiveModeChange(root, mode, 'agent');
        return textResult({
            ...result,
            insights: mode === 'B' ? result.insights : undefined,
        });
    });
    server.registerTool('get_cognitive_insights', {
    ...
    server.registerTool('get_skill_suggestions', {
    ...
    server.registerTool('get_model_preset', {
