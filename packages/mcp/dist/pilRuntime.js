import { z } from 'zod';
import { readStateJson, readWorkspaceStatus, readProjectBuiltState, readIntentGraphVNext, readGovernanceDecision, readDecisionProvenanceGraph, readProjectEvolutionTimeline, readProjectGraph, readConfidenceIndex, queryConfidenceIndex, readWhyLayer, readProjectIntelligenceHealth, deriveProjectIntelligenceHealth, readDecisionLog, loadTransferExportInput, buildTransferContextSnapshot, buildFullIntelligenceMarkdown, formatTransferContextMarkdown, toTransferContextPayload, estimateGovernanceTokens, getProjectHandoff, captureProjectFocus, captureProjectNote, captureProjectDecision, retrieveImpact, retrieveEvolution, retrieveProvenance, } from '@contora/state-core';
...
export function registerPilRuntimeTools(server, resolveRoot) {
    ...
    server.registerTool('inspect_timeline', {
    ...
    server.registerTool('inspect_confidence', {
    ...
    server.registerTool('inspect_health', {
    ...
    server.registerTool('inspect_why', {
    ...
    server.registerTool('inspect_impact', {
    ...
    server.registerTool('inspect_evolution', {
    ...
    server.registerTool('inspect_provenance', {
    ...
    const transferHandoffHandler = async ({ workspaceRoot: override }) => {
        const root = override ? override : await resolveRoot();
