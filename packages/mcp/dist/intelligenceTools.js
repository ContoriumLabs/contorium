import { z } from 'zod';
import { readDecisionProvenanceGraph, readGovernanceDecision, readImpactGraph, readIntentGraphVNext, readProjectEvolutionTimeline, readProjectIdentity, readConfidenceIndex, readProvenanceChain, readEvolutionGraph, readWhyLayer, queryImpactGraph, queryProjectEvolutionTimeline, queryConfidenceIndex, queryProvenanceChain, queryEvolutionGraph, readProjectIntelligenceHealth, deriveProjectIntelligenceHealth, readDecisionLog, loadTransferExportInput, buildTransferContextSnapshot, buildFullIntelligenceMarkdown, formatTransferContextMarkdown, toTransferContextPayload, estimateGovernanceTokens, 
/** @deprecated legacy alias */
readStabilityIndex, queryStabilityIndex, } from '@contora/state-core';
 ...
 */
export function registerIntelligenceTools(server, resolveRoot) {
    server.registerTool('get_project_identity', {
        description: '[Project Intelligence · inspect] Cross-tool project identity (.contora/identity/project_identity.json).',
    ...
    server.registerTool('get_project_decision', {
        description: '[Project Intelligence · inspect] Decision provenance graph + latest governance decision record.',
        inputSchema: workspaceRootSchema,
    }, async ({ workspaceRoot: override }) => {
        const root = override ? override : await resolveRoot();
    ...
    server.registerTool('get_project_why', {
    ...
    server.registerTool('get_project_intent_graph', {
    ...
    server.registerTool('get_project_evolution_timeline', {
        description: '[Dimension · TIMELINE · inspect] Structured evolution history (.contora/timeline/project_timeline.json). Descriptive — not a log dump.',
    ...
    server.registerTool('get_impact_graph', {
        description: '[Dimension · IMPACT · inspect] Scope and propagation model (.contora/graph/impact_graph.json). Descriptive — not risk prediction.',
    ...
    const confidenceHandler = async (args) => {
        const root = args.workspaceRoot ? args.workspaceRoot : await resolveRoot();
    ...
    server.registerTool('get_confidence_index', {
        description: '[Dimension · CONFIDENCE · inspect] Trustworthiness of recorded intelligence (.contora/confidence/confidence_index.json).',
        inputSchema: entityQuerySchema,
    }, confidenceHandler);
    server.registerTool('get_stability_index', {
    ...
    server.registerTool('get_provenance_chain', {
        description: '[System · PROVENANCE · inspect] Trace-back chains WHY → DECISION → INTENT → TIMELINE (.contora/provenance/provenance_chain.json).',
    ...
    server.registerTool('get_evolution_graph', {
        description: '[System · EVOLUTION · inspect] Structured transformation chains (.contora/evolution/evolution_graph.json). Not chronological timeline.',
        inputSchema: anchorQuerySchema,
    }, async (args) => {
        const root = args.workspaceRoot ? args.workspaceRoot : await resolveRoot();
    ...
    server.registerTool('get_project_intelligence_health', {
        description: '[v1.1.3 · inspect] Intelligence completeness, weighted health_score, knowledge_coverage (.contora/intelligence/health.json). Measures asset completeness — not project quality.',
        inputSchema: workspaceRootSchema,
    }, async ({ workspaceRoot: override }) => {
        const root = override ? override : await resolveRoot();
    ...
    server.registerTool('get_decision_log', {
        description: '[v1.1.3 · inspect] Append-only decision log (.contora/decision/decision_log.json). Records selected alternatives — not recommendations.',
        inputSchema: workspaceRootSchema,
    }, async ({ workspaceRoot: override }) => {
        const root = override ? override : await resolveRoot();
    ...
    server.registerTool('get_cognitive_snapshot', {
        description: '[Legacy alias · prefer transfer_context] Compressed Cognitive Snapshot (~300–800 tokens).',
    ...
    server.registerTool('get_full_intelligence', {
        description: '[Legacy alias · prefer transfer_intelligence] Full Project Intelligence export (~8000 tokens).',
        inputSchema: workspaceRootSchema,
    }, async ({ workspaceRoot: override }) => {
        const root = override ? override : await resolveRoot();
        const input = await loadTransferExportInput(root);
