...
server.registerTool('get_intent_graph', {
    description: '[Legacy · prefer inspect_intent] Old intent graph at .contora/intent-graph/graph.json (not vNext). Use inspect_intent for .contora/intent/intent_graph.json.',
    inputSchema: z.object({
        workspaceRoot: z.string().optional().describe('Override workspace root; default auto-detect'),
    }),
}, async ({ workspaceRoot: override }) => {
...
server.registerTool('get_project_intent', {
    description: '[Legacy · prefer inspect_intent] Compact intent summary artifact. Prefer inspect_intent (vNext graph) or ask_project for NL.',
    inputSchema: z.object({
        workspaceRoot: z.string().optional().describe('Override workspace root; default auto-detect'),
    }),
}, async ({ workspaceRoot: override }) => {
...
server.registerTool('get_handoff_injection_status', {
    description: '[Prefer · New chat] Call at the start of a new session: check if runtime handoff injection is pending. If pending=true, ask the user then call confirm_handoff_injection (Y) or skip_handoff_injection (N). Else use transfer_project(mode=context) if continuity is needed.',
...
server.registerTool('confirm_handoff_injection', {
    description: '[Write · New chat] After user confirms (Y): write .contora/mcp.auto-context.md and mark injection done. Optional format: json | markdown | compact (default markdown). Side effect: persists context file.',
    inputSchema: z.object({
        workspaceRoot: z.string().optional().describe('Override workspace root; default auto-detect'),
        format: z
...
server.registerTool('skip_handoff_injection', {
    description: '[Write · New chat] User declined injection (N). Marks skip for this runtime session; get_project_handoff / transfer_project remain available on demand.',
    inputSchema: workspaceRootSchema,
}, async ({ workspaceRoot: override }) => {
    const root = override ? path.resolve(override) : await workspaceRootForTools();
